<section class="rates container">
    ...
    <?php foreach ($bets as $bet): ?>
        ...
        <?php $is_winner = $bet['winner_id'] == getUserId(); ?>
        ...
        <tr class="rates__item <?=$end_classname; ?> <?=$winner_classname ?>">
            ...
            <td class="rates__timer">
                <?php $finishing_classname = getDtRange($bet['date_close'])[0] < 24 && !$is_end && !$is_winner ? "timer--finishing" : ""; ?>
                <?php $end_classname = $is_end && !$is_winner ? "timer--end" : ""; ?>
                <?php $winner_classname = $is_winner ? "timer--win" : ""; ?>
                <div class="timer <?=$end_classname; ?><?=$winner_classname; ?><?=$finishing_classname; ?>"><?=getBetInfo($bet, getUserId()); ?></div>
            </td>
            <td class="rates__price">
            <?=$bet['rate']; ?> р
            </td>
            <td class="rates__time">
            <?=getPassedTime($bet['date_add']); ?>
            </td>
        </tr>
    <?php endforeach; ?>
    </table>
